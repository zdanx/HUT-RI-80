...
let existingCelebrationDocId = null;
...
// Function to update user name display in the main content (now also for login status)
function updateUserNameDisplay() {
    const userNameDisplayMain = document.getElementById('user-name-display-main'); // This one will be removed soon.
    if (userNameDisplayMain && currentUserName) {
        // This is the old display, it will be removed.
    ...
    // Update the new login status display
    if (loginStatusDisplay) {
        if (currentUserName) {
            loginStatusDisplay.textContent = `Login sebagai: ${currentUserName}`;
        } else if (currentUserId) {
            loginStatusDisplay.textContent = `Login sebagai: Anonim (${currentUserId.substring(0, 5)}...)`;
        } else {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // const userNameDisplayMain = document.getElementById('user-name-display-main'); // No longer directly used here
        ...
        onAuthStateChanged(auth, async (user) => {
            ...
            if (user) {
                ...
                if (userProfileSnap && userProfileSnap.exists() && userProfileSnap.data().name) {
                    ...
                    updateUserNameDisplay(); // Use the unified function
    ...
    async function checkIfAlreadyCelebrated() {
        ...
        try {
            ...
            if (!querySnapshot.empty) {
                hasCelebrated = true;
                existingCelebrationDocId = querySnapshot.docs[0].id;
                if (celebrateButton) {
        ...
        celebrateButton.addEventListener('click', async () => {
            ...
            try {
                ...
                if (!userImagesSnapshot.empty) {
                    ...
                    userImageUrl = userImages[0].url;
                }
            } catch (error) {
                console.error("Error fetching user image for celebration:", error);
            }

            try {
                const celebratorsCollectionRef = collection(db, `artifacts/${appId}/public/data/celebrators`);

                if (hasCelebrated && existingCelebrationDocId) {
                    const docRef = doc(celebratorsCollectionRef, existingCelebrationDocId);
                    await setDoc(docRef, {
                        userName: currentUserName,
                        timestamp: serverTimestamp(),
                        userImageUrl: userImageUrl || null
                    }, { merge: true });
                    showMessageBox("Perayaan Anda telah diperbarui!", "success");

                } else {
                    const newDocRef = await addDoc(celebratorsCollectionRef, {
                ...
                console.error("Error recording or updating celebration:", error);
                showMessageBox("Gagal mencatat perayaan. Coba lagi.", "error");
            }
        });
    }

    async function deleteImage(imageId, imageUserId) { // Pass imageUserId as argument
        if (!currentUserId || !db) {
        ...
        if (currentUserId !== imageUserId) {
            showMessageBox("Anda hanya dapat menghapus gambar yang Anda unggah sendiri.", "error");
            return;
        }
    ...
    function renderGallery(images) {
        ...
        images.forEach(imgData => {
            ...
            // Only add delete button if the current user is the uploader
            if (currentUserId === imgData.userId) {
                ...
                deleteButton.addEventListener('click', () => {
                    deleteImage(imgData.id, imgData.userId); // Pass both ID and uploader's userId
                });
    ...
    function renderCelebrators(celebrators) {
        ...
        celebrators.forEach(celebratorData => {
            ...
            if (celebratorData.userImageUrl) {
                ...
                imgElement.className = 'w-16 h-16 rounded-full object-cover mx-auto mb-2 border-2 border-red-500 shadow-md';
                imgElement.onerror = (event) => {
            ...
            if (imageUrl) {
                try {
                    ...
                    await addDoc(galleryCollectionRef, {
                        url: imageUrl,
                        userId: currentUserId, // Store the uploader's userId
                        userName: currentUserName,
                        timestamp: serverTimestamp()
                    });
                    imageUrlInput.value = '';
            ...
            if (name) {
                ...
                if (currentUserId && db) {
                     ...
                     try {
                         ...
                         updateUserNameDisplay(); // Update display after saving name
                     } catch (error) {
                ...
                if (nameModalOverlay) nameModalOverlay.classList.remove('show');
                updateUserNameDisplay(); // Update display after closing modal
            } else {
